...
- **Rule (Single Authoritative List Fetch)**: `ListViewEngine` MUST execute **exactly one** authoritative data query during initial mount. Never execute a speculative `fetch` to read metadata/fields followed immediately by `doFetch()`.
- **Rule (Cell-Level Request Deduplication)**: Custom cell controls (such as `UserControl`, lookup pickers, relation chips) rendered across table rows MUST use module-level in-flight Promise deduplication and caching (`fetchCached` with TTL). Never dispatch per-row raw `fetch` calls without deduplication.
- **Rule (Null-Safe Column & Primary Link Resolution)**: List view table and mobile renderers (`ListViewTable`, `ListViewMobile`) MUST implement defensive null-checks when resolving columns and primary links (`col?.fieldId`). If columns/fields have not yet loaded or are empty, fall back gracefully (`rec.name || rec.id || '\u2014'`) without throwing runtime errors.
- **Rule (Related Mode Fields Propagation)**: When fetching related records (`/api/dynamic/[table]/related`), the client engine MUST always extract and set `fields` alongside `rows` and `total` to ensure columns render properly in embedded and mobile views.
- **Rule (Parent-to-Child Layout Propagation)**: Dynamic page shells (`DynamicTablePage`) MUST pass pre-resolved layouts (`initialLayout`) to child engines (`ListViewEngine`) to eliminate redundant HTTP round-trips to `/api/console/layouts`.
...
- **Rule (PostgreSQL Single-Socket Pipeline Guard)**: NEVER execute concurrent queries via `Promise.all([client.query(...), client.query(...)])` on a single `pg.PoolClient` connection socket within a transaction. Execute queries sequentially with `await` to prevent connection pipeline lock contention.
...
- **Rule (React Hook Ordering & Early Return Guard)**: NEVER declare React hooks (`useMemo`, `useState`, `useEffect`, `useCallback`, etc.) below early conditional return statements (such as `if (fetchLoading) return ...` or `if (error) return ...`). All React hooks MUST be placed in the unconditional top-level declaration block of the component to prevent breaking the Rules of Hooks (`Rendered more hooks than during previous render`).