...
async function resetPlatform() {
  ...
  try {
    if (phase === 'all' || phase === '1') {
      ...
      console.log("🌱 [PHASE 1] Provisioning 'SAILS Default' tenant via API...");
      const provisionResponse = await fetch(`${API_URL}/api/tenant/provision`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          name: 'SAILS Default',
          adminEmail: 'admin@sails.app'
        })
      });
      ...
      console.log("\n✅ Phase 1 Complete. RESTART the 'sails-core' service before running Phase 2.");
    }
