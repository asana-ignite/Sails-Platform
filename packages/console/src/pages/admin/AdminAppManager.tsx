...
import { ConsoleApp } from '@sails/shared';
...
const AdminAppManager: React.FC = () => {
  ...
  return (
    <div className="sails-app-manager">
      <div className="sails-app-manager__actions" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        <button 
      ...
      <div className="sails-app-grid">
        {visibleApps.map(app => (
          <div key={app.id} className="sails-app-card">
            <div className="sails-app-card__icon">
              <DynamicIcon name={app.icon || 'Box'} size={24} />
            </div>
            <div className="sails-app-card__info">
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <h3 style={{ margin: 0 }}>{app.name}</h3>
                {app.isSystem && (
                  <span style={{ fontSize: '0.72rem', background: 'rgba(59,130,246,0.15)', color: 'var(--sails-primary, #3b82f6)', padding: '2px 6px', borderRadius: '4px', fontWeight: 600, display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
              ...
              {app.requiredCapability && (
                <div className="sails-app-card__capability" style={{ marginTop: '6px' }}>
                  <Shield size={12} />
                  <span>{app.requiredCapability}</span>
                </div>
              )}
            </div>
            <div className="sails-app-card__actions">
            ...
            <form onSubmit={handleSave}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '16px' }}>
                <div className="sails-form-group">
                  <label className="sails-label" style={{ display: 'block', marginBottom: '6px' }}>Icon (Lucide)</label>
                  <input 
                    type="text"
                    className="sails-input"
                    style={{ width: '100%' }}
