# INIDOS Console — Implementation Skill Guide

This document outlines the architectural patterns, design standards, and implementation roadmap for building the **INIDOS Console**. This guide serves as the long-term source of truth for AI agents working on this codebase.

## 1. Core Principles
- **Headless First**: The Console is a thin, premium UI layer. Logic for data structures and validation lives in **INIDOS Core** (Backend).
...
- **BEM Prefix**: `inidos-` (e.g., `.inidos-card`, `.inidos-card__header`, `.inidos-btn--primary`).
...
```jsx
<div className="inidos-layout">
  <Topbar className="inidos-layout__topbar" />
  <Sidebar className="inidos-layout__sidebar" />
  <main className="inidos-layout__main">
    <div className="inidos-page">
      <div className="inidos-container">
        {/* Dynamic Components */}
      </div>
    </div>
  </main>
  <Footer className="inidos-layout__footer" />
</div>
```
...
> **Load when:** Creating UI components, defining layouts, or writing CSS for the `inidos-console` project.
...
- **BEM Methodology:** You MUST write Vanilla CSS using BEM prefixed with `inidos-` (e.g., `.inidos-card`, `.inidos-card__header`, `.inidos-btn--primary`).
...
## After Develop, conduct a test in Docker name it `inidos-template` and share the test URL
